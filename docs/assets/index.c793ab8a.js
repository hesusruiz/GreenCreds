var e=Object.defineProperty,i=(i,t,s)=>(((i,t,s)=>{t in i?e(i,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):i[t]=s})(i,"symbol"!=typeof t?t+"":t,s),s);import{D as t,i as s,h as o,T as a,a as r,B as n,N as d}from"./vendor.de83c32c.js";class c extends Error{constructor(...e){super(...e),Error.captureStackTrace&&Error.captureStackTrace(this,c),this.name="Warning"}}async function l(e,i,...t){var s={timestamp:Date.now(),level:e,desc:i};t.length>0&&(s.more=t);try{await p.logs.add(s)}catch(a){console.error("Error in log add")}if(!(await p.logs.count()<=1e3)){var o=await p.logs.orderBy("id").first();try{await p.logs.delete(o.id)}catch(a){console.error("Error in log prune")}}}var v={mywarn:async function(e,...i){{let t=e;try{t=new c(e).stack}catch{}console.warn(e,...i),l("W",t,...i)}},myerror:async function(e,...i){let t=e;try{t=new Error(e).stack}catch{}console.error(t,...i),l("E",t,...i)},recentLogs:async function(){return await p.logs.reverse().limit(200).toArray()},clearLogs:async function(){await p.logs.clear(),alert("Logs cleared"),location.reload()}};const p=new t("SafeIslandNew");async function u(e,i){try{await p.settings.put({key:e,value:i})}catch(t){console.error(t),alert("Error in put setting")}}p.version(.5).stores({credentials:"hash, timestamp, type",settings:"key",logs:"++id, timestamp"});const g=s`
...
`;class y extends o{constructor(){console.log("Inside constructor of HEADERBAR"),super()}render(){return a`
        <div class="w3-bar w3-xlarge w3-indigo">
          <div class="w3-bar-item" @click=${k}>SafeIsland</div>
        ...
        `}enter(){console.log("Enter page DEMO"),this.style.display="block",this.requestUpdate()}};let L=M;i(L,"styles",[M.styles,s`
        .plus {
            --t:3px;   /* Thickness */
        ...
        `]),customElements.define("passenger-home",L);customElements.define("demo-page",class extends R{static get properties(){return{perico1:{type:Object},perico2:{attribute:!1}}}constructor(){console.log("Inside constructor of DEMO PAGE"),super(),this.perico1="default 111111",this.perico2="default 222222"}render(){return a`
        <!-- =========================================== -->
        ...
        `);return t}(this.hcert)}catch(i){return void v.myerror("Error rendering credential",i)}this.requestUpdate()}async exit(){}}i(N,"styles",[g,s`
        #footer {
            position: fixed;
            bottom: 0
        }
        `]),customElements.define(B,N);class V extends o{static get properties(){return{pepe:{attribute:!0}}}constructor(){console.log("Inside constructor of SCANRESULT"),super(),this.thehtml="",this.qrContent="",this.hcert=void 0,this.verified=!1}render(){return a`
            <!-- =========================================== -->
            <!-- SCAN RESULT PAGE.                           -->
            <!-- =========================================== -->
            ${this.thehtml}
            `}async enter(e){console.log("PRESENT Enter",e),this.qrContent="HC1:NCFOXN%TSMAHN-HPO45JVLFDISVM85:D4*OV-36QHN-TM5*KQM3*GML0FXIKMWDWYPWHH:ZH6I1$4JN:IN1MPK9V L9L69UEG%6M415BEC4633F-96SW6SJE3ZMXDMUF6NZ6E6AH+932Q6G39ZILAPZXI$MI1VCSWC%PDMOL7AD.XIIXB0 J5QB06JOMI/LKW1JVTIM7JZIJI7JIZI.EJJ14B2MZ8DC8CWVD 8D*NI+PB/VSQOL9DLKWCZ3E7KDW0KZ J$XI4OIMEDTJCJKDLEDL9CZTAKBI/8D:8DKTDL+SQ05.$S6ZCJKBPKJDG3LWTXD3/9TL4T.B9GYP8T1+1V$LA*ZEBH1T+UZ%HQK9CZPME1.*UWKU/.1ZY9 UPG706ZRB KYZQBK9-RUUBCFRMLNKNM8JI0JPGO7HWPAG2B5VL3K9OTJ0UL4ZVBWSA81TXH4YVZ38+APZGJKDJB1PGB49WE*7CY2P3LM DOLQ9KKAX15BP3:GNSFKPOQU-0DCOTCPHXOP40SF355",this.style.display="block";try{this.hcert=await j.decodeHC1QR(this.qrContent,!0),this.verified=this.hcert[3]}catch(i){return void v.myerror("Error verifying credential",i)}try{this.thehtml=this.renderDetail(this.hcert)}catch(i){return void v.myerror("Error rendering credential",i)}this.requestUpdate()}async exit(){}toggleView(e){this.renderRoot.querySelector(e).classList.toggle("w3-show")}renderDetail(e){let i=e[1],t="Unrecognized";return"v"==i.certType&&(t=a`
        ...
        `]),customElements.define("display-hcert",V);class U extends o{constructor(){console.log("Inside constructor of PAGE404"),super()}render(){return a`
        <div class="jrmpage w3-container">
            <h1>The page does not exist!</h1>
        </div>
        `}enter(){console.log("Enter page"),this.style.display="block",this.requestUpdate()}}i(U,"styles",[g]),customElements.define("page-404",U);var q=[{name:"demo",tagName:"demo-page"},{name:"passenger",tagName:"passenger-home"},{name:"verifier",tagName:"scanqr-page"},{name:"scanresult",tagName:"scanqr-result"},{name:"displayhcert",tagName:"display-hcert"},{name:"page404",tagName:"page-404"}];document.body.innerHTML="";let O=document.createElement("router-elem");document.body.append(O);for(let F=0;F<q.length;F++){let e=document.createElement(q[F].tagName);e.style.display="none",document.body.append(e),Y=q[F].name,K=e,console.log("ROUTER: register page:",Y),w.set(Y,K)}var Y,K,Q="displayhcert";if(window.location.search.length>0){let e=new URLSearchParams(window.location.search);null!==e.get("home")?Q="passenger":null!==e.get("verifier")?Q="verifier":null!==e.get("demo")?Q="demo":null!==e.get("admin")?Q="admin":null!==e.get("pubcred")?Q="pubcreds":v.mywarn("URL parameter not recognised",window.location.search)}console.log("Home_page:",Q),m=Q;const H="https://safeislandtest.hesusruiz.org";window.addEventListener("load",(async e=>{await async function(){if(console.log("Performing OneTime Initialization"),1!=await async function(e){try{var i=await p.settings.get(e)}catch(t){console.error(t),alert("Error in get setting")}if(null!=i)return i.value}("initialized")){try{await u("apiHost",H)}catch(e){v.myerror("Onetime initialization error")}await u("initialized",!0)}}();let i=window.location.search,t=new URLSearchParams(i),s=t.get("id");if(s){var o="https://api.lanzarotesafe.org/sms/"+s;await requestQRAndDisplay(o,"displayCredentialPage","fromPassengerScan")}else{if(s=t.get("pubid"),s){o="https://safeislandtest.hesusruiz.org/api/verifiable-credential/v1/public/"+s;return console.log("Public Credential",o),void(await requestQRAndDisplay(o,"displayCredentialPage","fromPassengerScan"))}await k()}}));
