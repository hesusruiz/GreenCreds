var e=Object.defineProperty,i=(i,t,s)=>(((i,t,s)=>{t in i?e(i,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):i[t]=s})(i,"symbol"!=typeof t?t+"":t,s),s);import{D as t,i as s,h as o,T as a,B as r,N as n}from"./vendor.6f639057.js";class d extends Error{constructor(...e){super(...e),Error.captureStackTrace&&Error.captureStackTrace(this,d),this.name="Warning"}}async function c(e,i,...t){var s={timestamp:Date.now(),level:e,desc:i};t.length>0&&(s.more=t);try{await v.logs.add(s)}catch(a){console.error("Error in log add")}if(!(await v.logs.count()<=1e3)){var o=await v.logs.orderBy("id").first();try{await v.logs.delete(o.id)}catch(a){console.error("Error in log prune")}}}var l={mywarn:async function(e,...i){{let t=e;try{t=new d(e).stack}catch{}console.warn(e,...i),c("W",t,...i)}},myerror:async function(e,...i){let t=e;try{t=new Error(e).stack}catch{}console.error(t,...i),c("E",t,...i)},recentLogs:async function(){return await v.logs.reverse().limit(200).toArray()},clearLogs:async function(){await v.logs.clear(),alert("Logs cleared"),location.reload()}};const v=new t("SafeIslandNew");async function p(e,i){try{await v.settings.put({key:e,value:i})}catch(t){console.error(t),alert("Error in put setting")}}v.version(.5).stores({credentials:"hash, timestamp, type",settings:"key",logs:"++id, timestamp"});const u=s`
...
`;class g extends o{constructor(){console.log("Inside constructor of HEADERBAR"),super()}render(){return a`
        <div class="w3-bar w3-xlarge w3-indigo">
          <div class="w3-bar-item" @click=${h}>SafeIsland</div>
        ...
        `}enter(){console.log("Enter page DEMO"),this.style.display="block",this.requestUpdate()}};let M=R;i(M,"styles",[R.styles,s`
        .plus {
            --t:3px;   /* Thickness */
        ...
        `]),customElements.define("passenger-home",M);customElements.define("demo-page",class extends P{static get properties(){return{perico1:{type:Object},perico2:{attribute:!1}}}constructor(){console.log("Inside constructor of DEMO PAGE"),super(),this.perico1="default 111111",this.perico2="default 222222"}render(){return a`
        <!-- =========================================== -->
        ...
        `}enter(){console.log("Enter page DEMO",this.perico1,this.perico2),this.style.display="block",this.requestUpdate()}});var L="scanqr-page";class B extends o{static get properties(){return{pepe:{attribute:!0}}}constructor(){console.log("Inside constructor of SCANQR"),super(),this.codeReader=new r,this.videoInputDevices=[],this.selectedDeviceId=void 0,this.selectHtml=a``,this.resultObj=void 0,this.result=void 0,this.videoElem=void 0}render(){return a`
        <!-- =========================================== -->
        <!-- SCAN QR PAGE.                               -->
        <!-- =========================================== -->
        ...
        `);return t}(this.hcert)}catch(i){return void l.myerror("Error rendering credential",i)}this.requestUpdate()}async exit(){}}i(G,"styles",[u,s`
        #footer {
            position: fixed;
            bottom: 0
        }
        `]),customElements.define(L,G);class V extends o{static get properties(){return{pepe:{attribute:!0}}}constructor(){console.log("Inside constructor of SCANRESULT"),super(),this.thehtml="",this.qrContent="",this.hcert=void 0,this.verified=!1}render(){return a`
            <!-- =========================================== -->
            <!-- SCAN RESULT PAGE.                           -->
            <!-- =========================================== -->
            ${this.thehtml}
            `}async enter(e){console.log("PRESENT Enter",e),this.qrContent="HC1:NCFOXN%TSMAHN-HPO45JVLFDISVM85:D4*OV-36QHN-TM5*KQM3*GML0FXIKMWDWYPWHH:ZH6I1$4JN:IN1MPK9V L9L69UEG%6M415BEC4633F-96SW6SJE3ZMXDMUF6NZ6E6AH+932Q6G39ZILAPZXI$MI1VCSWC%PDMOL7AD.XIIXB0 J5QB06JOMI/LKW1JVTIM7JZIJI7JIZI.EJJ14B2MZ8DC8CWVD 8D*NI+PB/VSQOL9DLKWCZ3E7KDW0KZ J$XI4OIMEDTJCJKDLEDL9CZTAKBI/8D:8DKTDL+SQ05.$S6ZCJKBPKJDG3LWTXD3/9TL4T.B9GYP8T1+1V$LA*ZEBH1T+UZ%HQK9CZPME1.*UWKU/.1ZY9 UPG706ZRB KYZQBK9-RUUBCFRMLNKNM8JI0JPGO7HWPAG2B5VL3K9OTJ0UL4ZVBWSA81TXH4YVZ38+APZGJKDJB1PGB49WE*7CY2P3LM DOLQ9KKAX15BP3:GNSFKPOQU-0DCOTCPHXOP40SF355",this.style.display="block";try{this.hcert=await j.decodeHC1QR(this.qrContent,!0),this.verified=this.hcert[3]}catch(i){return void l.myerror("Error verifying credential",i)}try{this.thehtml=this.renderDetail(this.hcert)}catch(i){return void l.myerror("Error rendering credential",i)}this.requestUpdate()}async exit(){}toggleView(e){this.renderRoot.querySelector(e).classList.toggle("w3-show")}renderDetail(e){let i=e[1],t="Unrecognized";return"v"==i.certType&&(t=a`
        ...
        `]),customElements.define("display-hcert",V);class N extends o{constructor(){console.log("Inside constructor of PAGE404"),super()}render(){return a`
        <div class="jrmpage w3-container">
            <h1>The page does not exist!</h1>
        </div>
        `}enter(){console.log("Enter page"),this.style.display="block",this.requestUpdate()}}i(N,"styles",[u]),customElements.define("page-404",N);var U=[{name:"demo",tagName:"demo-page"},{name:"passenger",tagName:"passenger-home"},{name:"verifier",tagName:"scanqr-page"},{name:"scanresult",tagName:"scanqr-result"},{name:"displayhcert",tagName:"display-hcert"},{name:"page404",tagName:"page-404"}];let q=document.createElement("router-elem");document.body.replaceChildren(q);for(let H=0;H<U.length;H++){let e=document.createElement(U[H].tagName);e.style.display="none",document.body.append(e),O=U[H].name,Y=e,console.log("ROUTER: register page:",O),m.set(O,Y)}var O,Y,K="displayhcert";if(window.location.search.length>0){let e=new URLSearchParams(window.location.search);null!==e.get("home")?K="passenger":null!==e.get("verifier")?K="verifierScanQR":null!==e.get("demo")?K="demo":null!==e.get("admin")?K="admin":null!==e.get("pubcred")?K="pubcreds":l.mywarn("URL parameter not recognised",window.location.search)}console.log("Home_page:",K),y=K;const Q="https://safeislandtest.hesusruiz.org";window.addEventListener("load",(async e=>{await async function(){if(console.log("Performing OneTime Initialization"),await A.init(),await I.init(),1!=await async function(e){try{var i=await v.settings.get(e)}catch(t){console.error(t),alert("Error in get setting")}if(null!=i)return i.value}("initialized")){try{await p("apiHost",Q)}catch(e){l.myerror("Onetime initialization error")}await p("initialized",!0)}}();let i=window.location.search,t=new URLSearchParams(i),s=t.get("id");if(s){var o="https://api.lanzarotesafe.org/sms/"+s;await requestQRAndDisplay(o,"displayCredentialPage","fromPassengerScan")}else{if(s=t.get("pubid"),s){o="https://safeislandtest.hesusruiz.org/api/verifiable-credential/v1/public/"+s;return console.log("Public Credential",o),void(await requestQRAndDisplay(o,"displayCredentialPage","fromPassengerScan"))}await h()}}));
